...
let ballSpawnRate = 1000; // Taxa inicial de geração de bolinhas em milissegundos
...
function initGame() {
  ...
  ballSpawnRate = 1000; // Reseta a taxa de geração de bolinhas
  updateUI();
  ...
  // Inicia a geração de bolinhas
  spawnInterval = setInterval(spawnCircle, ballSpawnRate);

  // Inicia a animação das bolinhas
...
function gameOver() {
  ...
  fetch('/save-score-blitz', {
    ...
    .then((response) => {
      if (response.ok) {
        // Exibe o mini-menu com a pontuação final
...
function checkGoal() {
  if (current === goal) {
    ...
    // Reduz o intervalo de spawn das bolinhas
    ballSpawnRate = Math.max(300, ballSpawnRate - 100);

    // Atualiza o intervalo de geração de bolinhas
    clearInterval(spawnInterval);
    spawnInterval = setInterval(spawnCircle, ballSpawnRate);
